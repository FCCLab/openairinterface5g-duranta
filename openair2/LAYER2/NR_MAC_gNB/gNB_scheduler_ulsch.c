                          ...
                          int *remainUEs_ext)
{
  ...
  /* NS: slice range on sched params; n_rb_avail is slice size for PF fair-split. */
  ...
  if (use_slice) {
    for (int j = 0; j < n_cand; j++) {
      ...
      LOG_D(NR_MAC,
            ...
            frame,
            slot);
      reset_beam_status(&nrmac->beam_info, frame, slot, cand->beam_index, slots_per_frame, cand->alloc_dci_beam_new);
      reset_beam_status(&nrmac->beam_info, sched_frame, sched_slot, cand->beam_index, slots_per_frame, cand->alloc_sched_beam_new);
  ...
  while (max_dci > 0) {
    ...
    *next = fsn_get_max(*next, min_next);
    ...
    if (nr_mac->scheduler_type_ul == SCHE_NS) {
      ...
      sched = nr_ul_schedule(nr_mac, pp_pusch, candidates, n_cand, max_dci, num_beams, len, sched_frame, sched_slot, k2, NULL, NULL);
      LOG_D(NR_MAC,
