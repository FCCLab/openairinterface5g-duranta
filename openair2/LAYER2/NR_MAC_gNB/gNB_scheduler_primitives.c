...
{
  /* MAX_DCI_CORESET is only the FAPI DCI array size — never use it as the
   * per-slot schedule budget.
   *
   * CCE-scaling (e.g. duration-2 → ~32 CCEs → budget 8) lets every connected
   * UE get a DL DCI on every DL slot. Under TDD 7 DL : 2 UL that overloads
   * PUCCH HARQ-ACK (format0 ≤2 bits/occasion, scarce UL PRBs) → pucch0_DTX →
   * BLER → RLF / reestablishment storms after a brief goodput ramp.
   *
   * Keep the upstream soft cap of 4. NS slice visit rotation + SRB/retx
   * priority (nr_dl_slice_proportional_fair) ensure the 5th UE is not
 ...
 */
/*! \brief True when this HARQ retx can never fit in the UE's slice max_prb_ratio.
 *
 * Transient misses (CCE, max_num_ue, this-slot window) must NOT abort HARQ;
 * only a structurally impossible retx (rbSize > slice max) should.
 */
bool nr_ns_retx_exceeds_slice_max(const slice_scheduler_t *ss, const NR_UE_info_t *UE, int retx_rbSize)
{
  if (ss == NULL || UE == NULL || retx_rbSize <= 0)
    return false;

  const int total_prbs = slice_sch_get_total_prbs(ss);
  if (total_prbs <= 0)
    return false;
  ...
  if (slice_sch_get_slice_config(ss, ue_slice.sst, ue_slice.sd, NULL, NULL, NULL, NULL, &max_ratio) != 0)
    return false;

  const int slice_max_prbs = (int)(total_prbs * max_ratio + 0.5f);
  return retx_rbSize > slice_max_prbs;
}
...
bool prepare_initial_ul_rrc_message(gNB_MAC_INST *mac, nr_cell_sched_t *cell, NR_UE_info_t *UE)
{
  ...
  nr_mac_register_gnb_rlc_rlf_handler(UE->rnti);
...
static void nr_mac_gnb_rlc_rlf_handler(int rnti)
{
...
{
  nr_rlc_set_rlf_handler(rnti, nr_mac_gnb_rlc_rlf_handler);
}
