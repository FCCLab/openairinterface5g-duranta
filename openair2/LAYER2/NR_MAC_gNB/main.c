...
#include "NR_MAC_gNB/gNB_scheduler_dlsch_default_policies.h"
#include "NR_MAC_gNB/gNB_scheduler_ulsch_default_policies.h"
...
/*! \brief Bind DL/UL preprocessors and policy callbacks after gNB YAML is loaded (re-called from
 * gnb_config.c after set_slice_config()).
 *
 * Configures dl_rb_alloc and ul_rb_alloc based on scheduler_type_dl / scheduler_type_ul. */
void nr_mac_init_scheduler(gNB_MAC_INST *mac)
{
  ...
  if (mac->scheduler_type_dl == SCHE_NS)
    mac->dl_rb_alloc = nr_dl_slice_proportional_fair;
  else
    mac->dl_rb_alloc = nr_dl_proportional_fair;

  if (mac->scheduler_type_ul == SCHE_NS)
    mac->ul_rb_alloc = nr_ul_slice_proportional_fair;
  else
    mac->ul_rb_alloc = nr_ul_proportional_fair;

  LOG_I(NR_MAC,
        "MAC scheduler initialized: DL=%s (rb_alloc=%s) UL=%s (rb_alloc=%s)\n",
        mac->scheduler_type_dl == SCHE_NS ? "SCHE_NS" : "SCHE_PF",
        mac->scheduler_type_dl == SCHE_NS ? "nr_dl_slice_proportional_fair" : "nr_dl_proportional_fair",
        mac->scheduler_type_ul == SCHE_NS ? "SCHE_NS" : "SCHE_PF",
        mac->scheduler_type_ul == SCHE_NS ? "nr_ul_slice_proportional_fair" : "nr_ul_proportional_fair");
}
