  ...
  if (use_slice) {
    for (int j = 0; j < n; j++) {
      ...
      LOG_D(NR_MAC,
            ...
            frame,
            slot);
      reset_beam_status(&mac->beam_info, frame, slot, cand->alloc_beam_dir, slots_per_frame, cand->alloc_new_beam);
  ...
  int n_rb_sched[num_beams])
{
  if (mac->slice_scheduler_dl == NULL || slice_sch_get_num_slices(mac->slice_scheduler_dl) == 0) {
    nr_dl_schedule(mac, pp_pdsch, UE_list, max_num_ue, num_beams, n_rb_sched, NULL, NULL);
    return;
  }
  ...
  for (int beam_idx = 0; beam_idx < num_beams; beam_idx++) {
    ...
    for (int i = 0; i < num_ranges; i++) {
      ...
      nr_dl_schedule(mac, pp_pdsch, UEs_in_this_slice, max_num_ue, num_beams, n_rb_sched, &allocation[s], remainUEs);
...
void nr_dlsch_preprocessor(gNB_MAC_INST *mac, post_process_pdsch_t *pp_pdsch)
{
  ...
  if (mac->scheduler_type_dl == SCHE_NS)
    nr_dl_schedule_ns(mac, pp_pdsch, UE_info->connected_ue_list, max_sched_ues, num_beams, n_rb_sched);
  else
    /* slice_prb=NULL, remainUEs=NULL → full-band modular PF path */
    nr_dl_schedule(mac, pp_pdsch, UE_info->connected_ue_list, max_sched_ues, num_beams, n_rb_sched, NULL, NULL);
}
